...
void next() {
	...
	while (token = *src) {
		...
		if (token == '\n') {
			...
			// parse string literal, the only supported escape character is '\n', store the string literal into data
			last_pos = data;
	...
	// unit_unary()
	{
		...
		if (token == Num) {
			...
			*++text = token_val;
			...
			data = (char *)(((int)data + sizeof(int)) & (INT_MIN));
			expr_type = PTR;
		}
		else if (token == Sizeof) {
			// sizeof is actually an unary operator, only `sizeof(int)`, `sizeof(char)` and `sizeof(*...)` are supported
			match(Sizeof);
			match('(');
			expr_type = INT;
		...
		while (token >= level) {
			...
			if (token == Assign) {
				...
				*++text = MOD;
				...
				// postfix inc(++) and dec(--) we will increase the value to the variable and decrease it 
